...
\newcolumntype{K}{>{\hsize=1.35\hsize\raggedright\arraybackslash}X}
\newcolumntype{M}{>{\hsize=0.65\hsize\raggedright\arraybackslash}X}
...
Также используются сокращения \texttt{load\_red\_identity} и
\texttt{load\_blue\_identity}: они настраивают автора коммита
(\texttt{user.name} и \texttt{user.email}) как \texttt{red} и
\texttt{blue} соответственно.

\subsection{Автоматизированный запуск}

Воспроизведение истории выполняется скриптами из каталога
\texttt{lab/scripts}. Из корня проекта запускаются:

\begin{itemize}[leftmargin=1.5em]
    \item \texttt{./lab/scripts/build\_git.sh};
    \item \texttt{./lab/scripts/build\_svn.sh}.
\end{itemize}

Скрипты создают артефакты в \texttt{lab/out}, проверяют наличие входных
архивов \texttt{commit0.zip ... commit14.zip} и обязательных утилит
(\texttt{git}, \texttt{svn}, \texttt{svnadmin}, \texttt{unzip}).
...
\hline
Подготовка &
\cmdcell{OUT\_DIR="./lab/out"\\
GIT\_REPO\_DIR="\$OUT\_DIR/git-repo"\\
rm -rf "\$GIT\_REPO\_DIR"\\
git init -b master "\$GIT\_REPO\_DIR"\\
cd "\$GIT\_REPO\_DIR"\\
...
\begin{xltabular}{\linewidth}{|L{0.10\textwidth}|K|M|}
\hline
\rowcolor{white}
...
\hline
r2 &
\cmdcell{git checkout master\\
apply\_snapshot 2\\
git commit -m "r2"} &
Продолжение верхней красной ветки. \\
\hline
r3 &
\cmdcell{git checkout -b blue\\
load\_blue\_identity\\
apply\_snapshot 3\\
git commit -m "r3"} &
...
apply\_snapshot 13\\
git commit -m "r13"} &
Слияние \texttt{red-bottom -> master}, конфликтующие файлы приводятся к состоянию \texttt{commit13.zip}. \\
\hline
r14 &
...
Подготовка &
\cmdcell{OUT\_DIR="./lab/out"\\
SVN\_ROOT\_DIR="\$OUT\_DIR/svn"\\
SVN\_REPO\_DIR="\$SVN\_ROOT\_DIR/repo"\\
SVN\_WC\_DIR="\$SVN\_ROOT\_DIR/wc"\\
rm -rf "\$SVN\_ROOT\_DIR"\\
svnadmin create "\$SVN\_REPO\_DIR"\\
REPO\_URL="file://\$SVN\_REPO\_DIR"\\
svn mkdir "\$REPO\_URL/trunk" "\$REPO\_URL/branches"\\
-m "Init repository layout"\\
svn checkout "\$REPO\_URL/trunk" "\$SVN\_WC\_DIR"\\
cd "\$SVN\_WC\_DIR"} \\
\hline
\end{tabularx}
...
\texttt{git} и \texttt{svn}. Были отработаны создание и конфигурация
репозиториев, работа с ветками, переключение пользователей и разрешение
конфликтов при слиянии. Практика показала различия между распределённой
и централизованной моделями контроля версий и закрепила базовые команды
обеих систем.
